...
# tensorflow s3d code: torch.nn.BatchNorm3d(num_features, eps=1e-3,
# momentum=0.001, affine=True, track_running_stats=True)
...
class S3DG(nn.Module):
    def __init__(self, input_channel=3, gating=False, slow=False, num_cls=400):
    ...
    def forward(self, x, o_type=None):
        x = self.block1(x)
        x = self.block2(x)
        x = self.block3(x)
        x = self.block4(x)
...
if __name__ == "__main__":
    model = S3DG(gating=True, slow=False, num_cls=400)
    input_data = torch.FloatTensor(1, 3, 6, 224, 224)
    ...
    time_e = time.time()
    print("Average inference time is {}".format((time_e - time_s) / 1000))
